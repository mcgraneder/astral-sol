...
contract Lender is Context, AccessControlEnumerable, Payment {
    ...
    mapping(bytes => uint256) public loans;
    ...
    function borrow(
        address token,
        bytes calldata borrower,
        uint256 amount
    ) external returns (uint256) {
        require(hasRole(LENDER_ADMIN, _msgSender()), "Lender: not lender admin");
    ...
    function repay(
        ...
        if (erc20 == FEE_CURRENCY) {
            ...
            IERC20(erc20).safeTransferFrom(_msgSender(), address(this), amount - change);
        }
        return change;
    }
}