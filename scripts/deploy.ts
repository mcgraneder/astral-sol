...
const addTokensInConstructor = async(registry: string): Promise<IAddToConstructor> => {
  ...
  for (let symbol = 0; symbol < mintGatewaySymbols.length; symbol++) {
   ...
   const asset = await gatewayRegistry.getRenAssetBySymbol(mintGatewaySymbols[symbol])
   assetAddresses.push(asset)
  }
  return { mintGatwayAddresses, assetAddresses, mintGatewaySymbols }
}

const addTokensAdterDeployment = async(registry: string, bridge: RenBridge, deployer: SignerWithAddress): Promise<void> => {
...
async function main(registry: string, addInConstructor: boolean) {
  ...
  if (addInConstructor) {
    const { mintGatwayAddresses, assetAddresses, mintGatewaySymbols }= await addTokensInConstructor(registry)

    console.log(mintGatwayAddresses, assetAddresses, mintGatewaySymbols)
    Bridge = await ethers.getContractFactory("RenBridge") as RenBridge__factory;
    bridge = await Bridge.connect(deployer).deploy(
      registry, mintGatwayAddresses, assetAddresses, mintGatewaySymbols, true
    ) as RenBridge;
    ...
    await bridge.deployed();
     ...
     //2 deploy bridge
    Bridge = await ethers.getContractFactory("RenBridge") as RenBridge__factory;
    bridge = await Bridge.connect(deployer).deploy(registry, [], [], [], false) as RenBridge;
    await bridge.deployed();
